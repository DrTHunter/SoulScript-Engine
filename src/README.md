...
| File | Purpose |
|------|---------|
| `loop.py` | Main interactive CLI entrypoint. Loads profile, builds system prompt (base + memory + directives + notes), runs conversation loop with tool dispatch, metering, and saves state/summary on exit. |
