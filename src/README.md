...
| `data_paths.py` | Canonical data directory layout. Every module that reads or writes to `data/` imports paths from here. Defines per-profile paths (`state.json`, `journal.jsonl`, `burst_journal.jsonl`, `narrative.md`, etc.) and shared paths (`vault.jsonl`, `task_inbox.jsonl`, `creator_inbox.jsonl`, `boundary_events.jsonl`, `change_log.jsonl`). Auto-creates directories on access. |
