...
def test_types():
    ...
    check("config max_tool_calls locked", cfg.max_tool_calls_per_tick == 1)
    ...
    tic = TickOutcome(tick_index=3, steps_taken=2, tool_used="memory.recall")
    d = tic.to_dict()
    check("tick_outcome to_dict", d["tick_index"] == 3 and d["tool_used"] == "memory.recall")
...
def test_tick_max_one_tool_call():
    """Tick enforces max 1 tool call — second attempt is denied."""
    print("\n=== Tick: Max 1 Tool Call ===")

    profile = _make_profile()
    config = BurstConfig(profile="orion", max_steps_per_tick=4)

    # Step 0: tool call (allowed), step 1: second tool call (denied), step 2: stop
    client = MockClient([
        _tool_call("recall", {"scope": "orion", "limit": 5}, "First tool call"),
        _tool_call("search", {"query": "test"}, "Second tool call (should be blocked)"),
    ...
    try:
        ...
        check("tool_used is memory.recall", outcome.tool_used == "memory.recall",
              f"got {outcome.tool_used}")
        check("has tool_denied error",
              any("second tool call blocked" in e for e in outcome.errors),
...
def test_tick_disallowed_tool():
    ...
    try:
        ...
        check("tool_used is None (blocked)", outcome.tool_used is None)
...
def test_tick_disallowed_memory_action():
    ...
    try:
        ...
        check("tool_used is None", outcome.tool_used is None)
